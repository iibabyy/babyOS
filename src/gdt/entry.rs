...
impl GdtEntry {
	...
	pub fn new(
		...
		let raw_entry = RawGdtEntry::new().with_access_flags(access_flags).with_flags(flags);

		Self(raw_entry).with_base_address(base_address).with_limit(limit)
	}
...
pub struct GdtEntryFlags {
	...
	/// 1 sets the segment to 64-bit code mode. If this is set, the D/B bit must
	/// be 0.
	pub long_mode: bool,
