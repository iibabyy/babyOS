...
impl FrameAllocator {
	...
	pub fn allocate_frame(&mut self) -> Option<usize> {
		...
		let (index, block) = self.bitmap.iter().enumerate().find(|(_, bits)| **bits != u32::MAX)?;
	...
	pub fn reserve_region(&mut self, start_address: usize, end_address: usize) {
		// We round DOWN the start, and round UP the end to ensure we only reserve safe
		// frames e.g. if we receive 3000 and 10000, we reserve 0-12288 instead of
		// 4096-8192
	...
	pub fn free_region(&mut self, start_address: usize, end_address: usize) {
		// We round UP the start, and round DOWN the end to ensure we only free safe
		// frames e.g. if we receive 3000 and 10000, we only free 4096-8192 instead of
		// 0-12288
