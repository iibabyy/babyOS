...
mod multiboot;
pub use multiboot::{MultibootInfo, GRUB_MULTIBOOT_MAGIC};

mod pmm;
use pmm::GLOBAL_ALLOCATOR;
pub use pmm::{pmm_allocate_frame, pmm_deallocate_frame};
...
/// 
/// Note: the bootloader send this map when loading the kernel to describe what memory spaces are used (free) or not
pub fn init_physical_memory(mb_info: &MultibootInfo) {
	...
	while current_addr < end_addr {
		...
		// Note: we can't use size_of(MemoryMapEntry) because some computers send more infos than what we capture
