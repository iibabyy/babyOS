...
/// Switches the active screen to `new_index` and saves the current cursor/color
/// states
///
/// Returns the saved cursor/color states of the new screen, or None if
...
pub fn handle_shortcut_switch_screen(new_index: usize) {
	...
	let res =
		switch_screen(new_index, writer.column_position, writer.row_position, writer.color_code);
